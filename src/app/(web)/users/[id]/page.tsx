...
const UserDetails = (props: { params: { id: string } }) => {
  ...
  const fetchUserOrder = async () => getUserOrders(userId);
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR('/api/userorder', fetchUserOrder);
      ...
      <div className='grid md:grid-cols-12 gap-10'>
        ...
        <div className='md:col-span-8 lg:col-span-9'>
          ...
          <nav className='sticky top-0 px-2 w-fit mx-auto md:w-full md:px-5 py-3 mb-8 text-gray-700 border border-gray-200 rounded-lg bg-gray-50 mt-7'>
            <ol
              className={`${
                currentNav === 'orders' ? 'text-blue-600' : 'text-gray-700'
          ...
          {currentNav === 'orders' && userOrders && Array.isArray(userOrders) && (
            <Table
        ...
        {currentNav === 'amount' && userOrders && Array.isArray(userOrders) && (
         <Chart userOrders={userOrders} />
        )}
        </div>
      </div>
